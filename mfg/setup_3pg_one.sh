...
# perform the DMA speed test on device 1 and DDR0
#
if ./dma_test 1 ddr0 1048576; then
  echo "DMA test on DDR0 finished"
  echo ""
else
  echo "Error running the DMA test on DDR0 for device 1"
  exit 1
fi

# perform the DMA speed test on device 1 and DDR1
#
if ./dma_test 1 ddr1 1048576; then
  echo "DMA test on DDR1 finished"
  echo ""
else
  echo "Error running the DMA test on DDR1 for device 1"
  exit 1
fi
